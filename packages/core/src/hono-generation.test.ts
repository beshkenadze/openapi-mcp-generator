...
describe('Hono MCP Server Generation', () => {
  ...
  test('should generate Hono MCP server with multiple transport endpoints', async () => {
    ...
    const generator = new OpenAPIMcpGenerator({ debug: false, skipFormatting: true });
    const outputPath = resolve(outputDir, 'src', 'server.ts');
    
    await generator.generateFromOpenAPI(testSpec, outputPath, 'test-hono-mcp', 'hono');
    ...
    const serverContent = readFileSync(join(outputDir, 'src', 'server.ts'), 'utf8');
    
    // Check for key imports
    expect(serverContent).toContain('import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js"');
    expect(serverContent).toContain('import { StreamableHTTPTransport } from "@hono/mcp"');
    ...
    const packageContent = readFileSync(join(outputDir, 'package.json'), 'utf8');
    ...
    console.log('✅ Hono MCP server generated successfully with all required files');
  }, 30000);
  ...
  test('should generate valid tools for different HTTP methods', async () => {
    ...
    await generator.generateFromOpenAPI(testSpec, outputPath, 'methods-test-mcp', 'hono');

    const serverContent = readFileSync(join(methodsOutputDir, 'src', 'server.ts'), 'utf8');
    ...
    if (getItemIndex !== -1) {
      const nextToolIndex = serverContent.indexOf('mcpServer.registerTool(', getItemIndex + 1);
      const getSection = nextToolIndex !== -1 
